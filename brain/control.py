...
TOOLS = ("brain_search","brain_get_record","brain_get_related","brain_health","brain_rebuild_status")
...
def policy_hash(data):
    keys=("enabled","allowed_workspaces","sensitive_workspace_grants","allowed_tools","revoked")
...
class ControlStore:
    ...
    def connect(self):
        ...
        con.executescript(SCHEMA); con.execute("INSERT OR IGNORE INTO control_schema VALUES(1,?)",(now(),)); con.commit(); return con
    def _decode(self,row):
        if not row:return None
        d=dict(row)
        for k in ("allowed_workspaces","sensitive_workspace_grants","allowed_tools"):d[k]=json.loads(d[k])
        d["enabled"]=bool(d["enabled"]);d["revoked"]=bool(d["revoked"]);return IntegrationProfile(**d)
    ...
    def save(self,profile,actor="Pavol/operator",reason=None):
        ...
        values={**data,"enabled":int(profile.enabled),"revoked":int(profile.revoked),**{k:json.dumps(data[k],sort_keys=True) for k in ("allowed_workspaces","sensitive_workspace_grants","allowed_tools")}}

...
class RuntimeInspector:
    ...
    def inspect(self):
        ...
        events = []
        try: journal = _ro(self.config.journal_db_path); events = _event_rows(journal); out["journal_available"] = True
        except (OSError, sqlite3.Error): journal = None
        try: retrieval = _ro(self.config.retrieval_db_path); tables = _tables(retrieval); out["retrieval_db_available"] = "retrieval_documents" in tables

...
## Read scope and expansion safety

Every read tool (`brain_search`, `brain_get_record`, `brain_get_related`) resolves scope through `policy.resolve_scope()`/`policy.authorize()` and passes the resulting workspace list (and, where relevant, sensitive-workspace grants) explicitly into the `Brain` call — there is no implicit or default scope on the MCP path. At the library layer, `Brain.get_record`/`Brain.get_related` require an explicit `allowed_workspaces` keyword argument; omitting it raises `TypeError` immediately, before any journal or retrieval query runs. `Brain.search` enforces the same thing through `SearchRequest` validation (`workspaces` must be explicit and non-empty). A separate trusted sentinel, `brain.api.UNRESTRICTED_OPERATOR_SCOPE`, exists only for local operator/diagnostic tooling that calls `Brain` directly (outside MCP) and needs to inspect a record whose workspace isn't known in advance — it is never imported or referenced anywhere in `brain/mcp_server.py`, and no MCP tool can select it.

Retrieval expansion never exceeds direct access: any record id that appears anywhere in a response — a search result, an `include_artifacts` link, a `get_related` row, or a `supersedes`/`superseded_by` provenance pointer — is guaranteed to be independently fetchable by that same caller through a direct, identically-scoped `get_record` call. This is enforced uniformly for every relation shape (incoming typed link, outgoing typed link, `record://` artifact target, `supersedes`, `superseded_by`) by one shared target-resolution check, so a corrupt or historical cross-workspace pointer — one the write path itself would refuse to create today — cannot be used to read a foreign record's id, workspace, or any other detail through expansion. A pointer to an out-of-scope, sensitive-without-grant, dangling, or rejected/forgotten target is simply absent from the response (the field is `null`, or the row is omitted) — never a distinguishing error, and never the foreign id itself. Canonical journal state is never modified by this filtering; only the response is sanitized.

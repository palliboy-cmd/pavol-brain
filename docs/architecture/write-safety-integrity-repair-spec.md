...
|---|---|---|
| Band C (secret/transcript) filter | Covers every client string **value** reachable from payload + metadata + provenance, including `idempotency_key`, `change_reason`, nested `verification` values and `alternatives[].evidence` (tested by `test_band_c_filters_all_persisted_client_text`). Does **not** scan dict **keys** — see B6 [probe]. `request_id` is also outside the gate — see B7. | `brain/write_policy.py:51-63` (`collect()` recurses into `value.values()` only) |
...
| Same agent, same explicit key, same payload, different metadata (`supersedes`/`links`/`valid_at`/provenance) | `BRAIN_IDEMPOTENCY_CONFLICT` (request_hash mismatch) | ✅ implemented — except B8 |
| …when the stored event lacks `request_hash` | `BRAIN_IDEMPOTENCY_CONFLICT` with `details.reason="legacy_record_without_request_hash"` — **never** a silent idempotent return | ❌ B8, fix required |
| Same agent, same explicit key, **different workspace or type** | `BRAIN_IDEMPOTENCY_CONFLICT` (content_hash mismatch under the same stored key). Rationale: an explicit key names one logical write; reusing it across workspaces/types is an agent bug and must fail loud. This is current behavior [probe] — now normative, add the pinning test. | ✅ behavior exists; test missing |
...
| Supersede replay | idempotent return of the original supersede result; target superseded exactly once (`_existing` precedes the supersede branch) | ✅ implemented; add explicit test |
...
| `verification` dict **keys** | agent-controlled | **not scanned, unconstrained** | **B6 — scan keys + constrain pattern** |
...
| `request_id` | control-adjacent | free string, unscanned, audit-logged | **B7 — constrain shape** |
...
- 2026-07-15 — **Package 1 implemented (closes B1 and B2; I1, I2, I3 satisfied).** Not yet committed as of this changelog entry — see the implementation report for exact git status. Evidence:

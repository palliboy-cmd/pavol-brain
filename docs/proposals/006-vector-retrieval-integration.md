...
2. **Slice 2 — incremental projection:** journal cursor, projection-hash reuse, embedding cache, supersede/forget invalidation, idempotent replay (§11).
   > **Completed 2026-07-12.** `brain/projector` implements the bounded, crash-safe incremental projection; artifact-relation eligibility comes only from the canonical validation fold defined in [Proposal 008](008-canonical-artifact-validation.md) (relation-level, append-only, filesystem reachability is diagnostic only — never eligibility evidence). Live gate on mini-core: 51 documents / 50 current / 51 embeddings, `rec-048` excluded from current projection by explicit human approval (`verified_inactive`/`wrong_target`), contract parity 24/24, zero leaks, second run `NO_CHANGES` with zero embedding requests, p95 39.3 ms, active `retrieval.db` and canonical journal untouched by projector runs. New artifact relations without a validation judgement stop projection with `REBUILD_REQUIRED` and the affected relation IDs; a validation-state change alone does not advance the journal cursor — it is picked up by the next record event or a fresh build. Evidence: `sqlite-spike/results/brain-slice2-projection.json`, `brain-slice2-live.json`, `brain-slice2-failure-recovery.json`.
